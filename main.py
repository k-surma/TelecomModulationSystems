...
def main():
    ...
    for snr_db in snr_values:
        # Po prostu wysyłamy BPSK z nominalnym snr_db
    ...
    for snr_db in snr_values:
        ...
        norm = 0.9 if snr_db<15 else 1.0
    ...
    plt.title("Porównanie BER: BPSK vs. QPSK (kara implementacyjna dla QPSK <10dB)")
    plt.grid(True)
    plt.legend()
    ...
    # --- (opcjonalnie) Sieć neuronowa do wyboru modulacji ---
    ...
    new_samples = np.array([
        [5,   0.2,   0.35,  0.8,   0.5   ],  # SNR=5 (niskie, QPSK ma karę)
